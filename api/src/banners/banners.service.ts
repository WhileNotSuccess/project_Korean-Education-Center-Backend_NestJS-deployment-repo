...
export class BannersService {
  ...
  async findAll(ignore:boolean) {
    ...
    if(!ignore){query.where('expiredDate >= NOW()')} 
    // sql문으로 NOW()가 현재 날짜,시간을 계산하여 현재 날짜보다 높은(나중인) banner들을 다 가져옴
    // ignore가 false 라면 expiredDate 열과 상관없이 모든 Banner를 불러옴 
  ...
  async update(id: number, updateBannerDto: UpdateBannerDto, file:Express.Multer.File) {
    await transactional(this.dataSource,async queryRunner=>{
      await queryRunner.manager.update(Banner,id,{...updateBannerDto,image:file.filename})
    })
  }
