...
export class CreateTables1745492894248 implements MigrationInterface {
    name = 'CreateTables1745492894248'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`Banner\` (\`id\` int NOT NULL AUTO_INCREMENT, \`image\` varchar(70) NOT NULL, \`language\` enum ('korean', 'english', 'japanese') NOT NULL DEFAULT 'korean', \`expiredDate\` datetime NOT NULL, \`url\` varchar(80) NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`course\` (\`id\` int NOT NULL AUTO_INCREMENT, \`korean\` varchar(20) NOT NULL, \`japanese\` varchar(20) NOT NULL, \`english\` varchar(20) NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`ApplicationForm\` (\`id\` int NOT NULL AUTO_INCREMENT, \`userId\` int NOT NULL, \`courseId\` int NOT NULL, \`createdDate\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`isDone\` tinyint NOT NULL DEFAULT 0, \`phoneNumber\` varchar(30) NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`posts\` (\`id\` int NOT NULL AUTO_INCREMENT, \`title\` varchar(255) NOT NULL, \`content\` longtext NOT NULL, \`userId\` int NOT NULL, \`category\` varchar(25) NOT NULL, \`createdDate\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedDate\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`language\` enum ('korean', 'english', 'japanese') NOT NULL DEFAULT 'korean', INDEX \`pagination_Index\` (\`category\`, \`language\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
